...
######################## change the input------sugar concentration in phloem increased 20%------
   ...
   DATA$Cpmin <- 75  *1.2                                    ## mmol/L
   DATA$Cpmax <- 275  *1.2                                   ## mmol/L
   DATA$Wpmax <- -1.5                                    ##  bar 
   DATA$Wpmin <- -7.5                                    ##  bar  
   ...
   DATA_input1 <- subset(DATA,dap >=4 & dap < 106,select=c(Tps,Temp,RH,Cp,PTLx))
  ...
  par(mfrow=c(1,2))
  plot(res$Tps/24, res$Stot, type='l',xlab='DAA',ylab='Dry weight (g)',ylim=range(res$Stot,res1$Stot,na.rm=T),lwd=2.5)
  lines(res1$Tps/24, res1$Stot, type='l',lwd=2.5,col=2)
   ...
   plot(res$Tps/24, res$FFW, type='l',xlab='DAA',ylab='Fresh weight (g)',ylim=range(res$FFW,res1$FFW,na.rm=T),lwd=2.5)
